...
export const getYoutubeChannelInfo = async (data) => {
...
export const getYoutubeVideosByIds = async (data) => {
    return await callYoutubeApi(
        `videos?key=${data.key}&id=${data.videoIds}&part=${data.part ? data.part : 'snippet,contentDetails,statistics,status'}&fields=${data.fields ? data.fields : '*'}&maxResults=${data.maxResults ? data.maxResults : '20'}${data.pageToken ? `&pageToken=${data.pageToken}` : ''}`,
    );
};
